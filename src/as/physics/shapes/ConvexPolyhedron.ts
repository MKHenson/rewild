...
export class ConvexPolyhedron extends Shape {
   ...
   * @param  {Vec3} target
   */
  getFaceNormal(i: f32, target: Vec3): void {
  ...
  findSeparatingAxis(
    ...
    for (let e0 = 0; e0 !== hullA.uniqueEdges.length; e0++) {
      ...
      for (const e1 = 0; e1 !== hullB.uniqueEdges.length; e1++) {
        // Get world edge 2
   ...
   */
  testSepAxis(axis: Vec3, hullB: ConvexPolyhedron, posA: Vec3, quatA: Quaternion, posB: Vec3, quatB: Quaternion): f32 {
    const hullA = this;
  ...
  clipFaceAgainstHull(
    ...
    for (let i: i32 = 0; i < hullA.faces.length; i++) {
      for (const j = 0; j < hullA.faces[i].length; j++) {
        if (
  ...
  transformAllPoints(offset: Vec3, quat: Quaternion): void {
    ...
    if (quat) {
      ...
      /*
        // Rotate edges
        for(const i=0; i<this.uniqueEdges.length; i++){
   ...
   */
  static project(hull: ConvexPolyhedron, axis: Vec3, pos: Vec3, quat: Quaternion, result: [f32, f32]): void {
