 ...
 * @author xprogram
 */
export class TetraTetraCollisionDetector extends CollisionDetector {
  constructor() {
    super();
  }

  detectCollision(tet1: Tetra, tet2: Tetra, manifold: ContactManifold): void {
    /*
     * What we are doing:
    ...
    let j1: Vec3,
      j2: Vec3,
      j3: Vec3,
      ts: i32 = 0; // Triangle vertices `j1`, `j2` and `j3`
  ...
  tricheck(p: Vec2, p0: Vec2, p1: Vec2, p2: Vec2): boolean {
    let A = 0.5 * (-p1.y * p2.x + p0.y * (-p1.x + p2.x) + p0.x * (p1.y - p2.y) + p1.x * p2.y);
    let sg = A < 0 ? -1 : 1;
    let s = (p0.y * p2.x - p0.x * p2.y + (p2.y - p0.y) * p.x + (p0.x - p2.x) * p.y) * sg;
    let t = (p0.x * p1.y - p0.y * p1.x + (p0.y - p1.y) * p.x + (p1.x - p0.x) * p.y) * sg;
