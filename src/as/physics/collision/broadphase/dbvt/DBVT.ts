...
export class DBVT {
  ...
  moveLeaf(leaf: DBVTNode): void {
    this.deleteLeaf(leaf);
    this.insertLeaf(leaf);
  }

  insertLeaf(leaf: DBVTNode): void {
    if (this.root == null) {
      this.root = leaf;
      return;
    }
    ...
    while (sibling.proxy == null) {
      ...
      const c1 = sibling.child1!;
      const c2 = sibling.child2!;
      const b = sibling.aabb;
      const c1b = c1.aabb;
      const c2b = c2.aabb;
      ...
      const incrementalCost = (newArea - oldArea) * 2; // cost of creating a new pair with the node
      let discendingCost1 = incrementalCost;
      ...
      let discendingCost2 = incrementalCost;
  ...
  getBalance(node: DBVTNode): f32 {
    if (node.proxy != null) return 0;
    return node.child1!.height - node.child2!.height;
  }

  deleteLeaf(leaf: DBVTNode): void {
    if (leaf == this.root) {
      this.root = null;
      return;
    }
    let parent = leaf.parent!;
    let sibling: DBVTNode | null;
  ...
  balance(node: DBVTNode): DBVTNode {
