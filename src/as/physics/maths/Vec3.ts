...
export class Vec3 {
  ...
  /**
   * Get the cross product matrix a_cross from a vector: Vec3, such that a x b = a_cross * b = c
   * @method crossmat
   * @see http://www8.cs.umu.se/kurser/TDBD24/VT06/lectures/Lecture6.pdf
   * @return {Mat3}
   */
  crossmat(): Mat3 {
    return new Mat3(f32Array([0, -this.z, this.y, this.z, 0, -this.x, -this.y, this.x, 0]));
  }
  ...
  /**
   * Multiply the vector with an other vector: Vec3, component-wise.
   * @method mult
   * @param {Number} vector
   * @param {Vec3} target The vector to save the result in.
   * @return {Vec3}
   ...
   * @return {Vec3}
   */
  addScaledVector(scalar: f32, vector: Vec3, target: Vec3): Vec3 {
    target = target || new Vec3();
