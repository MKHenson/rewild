...
export class Mat3 {
  ...
  constructor(elements: Float32Array = new Float32Array(9)) {
   ...
   * @return {Vec3}
   */
  getTrace(t: Vec3): void {
    const target = t || new Vec3();
    const e = this.elements;
    target.x = e[0];
    target.y = e[4];
    target.z = e[8];
  ...
  solve(b: Vec3, target: Vec3): Vec3 {
    ...
    let n: i32 = 3,
      k: i32 = n;
    const kp: i32 = 4; // num rows
    let p: i32, els: i32, np: i32;
    do {
      i = k - n;
      if (eqns[i + nc * i] === 0) {
   ...
   * @return {Number}
   */
  e(row: i32, column: i32, value: f32 | null): f32 {
    if (value == null) {
  ...
  copy(source: Mat3): Mat3 {
    for (let i: i32 = 0; i < source.elements.length; i++) {
  ...
  toString(): string {
    ...
    for (let i: i32 = 0; i < 9; i++) {
      r += (this.elements[i] + sep).toString();
    }
    return r;
  }
  ...
  reverse(target: Mat3 = new Mat3()): Mat3 {
    ...
    i = 2;
    do {
      const multiplier: f32 = 1 / eqns[i + nc * i];
      np = nc;
      do {
        p = nc - np;
