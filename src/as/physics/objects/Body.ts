 ...
 */
const wakeupEvent: Event = new Event("wakeup");

/**
 * Dispatched after a body has gone in to the sleepy state.
 */
const sleepyEvent = new Event("sleepy");

/**
 * Dispatched after a body has fallen asleep.
 */
const sleepEvent = new Event("sleep");
...
export enum BodyType {
  ...
  /**
   * A static body does not move during simulation and behaves as if it has infinite mass. Static bodies can be moved manually by setting the position of the static  The velocity of a static body is always zero. Static bodies do not collide with other static or kinematic bodies.
   */
  STATIC = 2,
...
export class Body extends EventDispatcher {
  ...
  /**
   * Set to true if you don't want the body to rotate. Make sure to run .updateMassProperties() after changing
   */
  ...
  wakeUp(): void {
    ...
    if (s === SleepState.SLEEPING) {
      this.dispatchEvent(wakeupEvent);
    }
  }
  ...
  sleepTick(time: f32): void {
    if (this.allowSleep) {
      ...
      if (sleepState === SleepState.AWAKE && speedSquared < speedLimitSquared) {
        ...
        this.dispatchEvent(sleepyEvent);
      } else if (sleepState === SleepState.SLEEPY && speedSquared > speedLimitSquared) {
        this.wakeUp(); // Wake up
      } else if (sleepState === SleepState.SLEEPY && time - this.timeLastSleepy > this.sleepTimeLimit) {
        this.sleep(); // Sleeping
        this.dispatchEvent(sleepEvent);
      }
    }
  }
  ...
  updateSolveMassProperties(): void {
    if (this.sleepState === SleepState.SLEEPING || this.type === BodyType.KINEMATIC) {
      this.invMassSolve = 0;
  ...
  */
  ...
  integrate(dt: f32, quatNormalize: boolean, quatNormalizeFast: boolean): void {
    ...
    if (
      !(this.type === BodyType.DYNAMIC || this.type === BodyType.KINEMATIC) ||
      this.sleepState === SleepState.SLEEPING
    ) {
      // Only for dynamic
      return;
