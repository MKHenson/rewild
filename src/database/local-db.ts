...
export function withSyncDefaults<T>(
  record: T & { id: string }
): StoredRecord<T> {
  return {
    updatedAt: 0,
...
export class LocalDataTable<T> {
  ...
  async getMany<Query = T>(q: IDataTableQuery<Query>) {
    const store = await this.openStore('readonly');
    let allItems = (await idbReq<StoredRecord<T>[]>(store.getAll())).map(
      withSyncDefaults
    );
    ...
    if (typeof q.cursor === 'number') cursor = q.cursor;
      ...
      cursor = filteredItems.findIndex(
        (item) => (item as unknown as { id: string }).id === q.cursor
      );
  ...
  async getDirty(): Promise<StoredRecord<T>[]> {
    const store = await this.openStore('readonly');
    const allItems = (await idbReq<StoredRecord<T>[]>(store.getAll())).map(
      withSyncDefaults
    );
  ...
  async markSynced(
    ...
    if (existing) {
      await idbReq(
        store.put({ ...withSyncDefaults(existing), syncedAt, syncError })
      );
    }
  }
