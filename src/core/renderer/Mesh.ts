...
export class Mesh extends Object3D {
  ...
  constructor(
    ...
    const pipelineInsPtr = wasm.createMeshPipelineInstance(
      this.pipeline.name,
      pipelineIndex
    );

    this.meshComponent = wasm.createMeshComponent(
      this.geometry.bufferGeometry as any,
      pipelineInsPtr,
      this.name
    );
    ...
    this.transformIndex = this.pipeline.addResourceInstance(
      renderer,
      GroupType.Transform
    );
    wasm.addComponent(this.transform as any, this.meshComponent as any);

    this.modelViewMatrix = wasm.getFloat32Array(
      wasm.getTransformModelViewMatrix(this.transform as any)
    );
    this.normalMatrix = wasm.getFloat32Array(
      wasm.getTransformNormalMatrix(this.transform as any)
    );
    this.worldMatrix = wasm.getFloat32Array(
      wasm.getTransformWorldMatrix(this.transform as any)
    );
  }
}
