...
import { AmbientLight, Component, DirectionalLight, Object3D, PhysicsComponent, wasm } from "packages/rewild-wasmtime";
...
export type LoaderPresetType = "crate" | "planet" | "ball" | "ambient-light" | "directional-light";

export const LoaderPresets: { [key in LoaderPresetType]: (actor: IActor, object: Object3D) => void } = {
  crate: (actor, object) => {
    const component = new PhysicsComponent(wasm.createBodyBox(0.5, 0.5, 0.5, 30));
    component.mass = 30;
  ...
  ball: (actor, object) => {
    ...
    const component = new PhysicsComponent(wasm.createBodyBall((geometry as SphereGeometry).parameters.radius, 1));
    component.mass = 1;

    const [x, y, z] = actor.properties.find((prop) => prop.type === "position")?.value as Vector3;
  ...
  "ambient-light": (actor, object) => {
    const [r, g, b] = actor.properties.find((prop) => prop.type === "color")?.value as Vector3;
    const light = object as AmbientLight;
    light.setColor(r, g, b);
    light.intensity = parseFloat(actor.properties.find((prop) => prop.type === "intensity")?.value as string);
  },
  "directional-light": (actor, object) => {
    const [r, g, b] = actor.properties.find((prop) => prop.type === "color")?.value as Vector3;
    const [x, y, z] = actor.properties.find((prop) => prop.type === "position")?.value as Vector3;
    const [tx, ty, tz] = actor.properties.find((prop) => prop.type === "target")?.value as Vector3;
