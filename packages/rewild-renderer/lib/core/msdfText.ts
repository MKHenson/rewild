...
export class MsdfText {
  ...
  constructor(
    ...
    this.bufferArray.set(new Matrix4().elements);
  ...
  setTransform(matrix: Matrix4) {
    this.bufferArray.set(matrix.elements);
    this.bufferArrayDirty = true;
  }
...
export class MsdfTextRenderer {
  ...
  updateCamera(projection: Matrix4, view: Matrix4) {
    this.cameraArray.set(projection.elements, 0);
    this.cameraArray.set(view.elements, 16);
    this.device.queue.writeBuffer(
      this.cameraUniformBuffer,
      0,
      this.cameraArray as BufferSource
    );
  }
