 ...
 *
 * These three live on the pass rather than in the uniform block because each
 * selects *pipeline* state: alphaMode picks the blend and depth-write state,
 * doubleSided picks the cull mode, and vertexColors picks the vertex layout and
 * entry point. The shader sees all three as well — a pipeline alone cannot
 * discard a fragment or mirror a back face's normal.
 *
...
export abstract class StandardPassBase implements IMaterialPass {
  ...
  /** Position, uv, normal, and COLOR_0 when the material asks for it. */
  ...
  protected vertexEntryPoint(): string {
    return this._vertexColors ? 'vsVertexColors' : 'vs';
  }
