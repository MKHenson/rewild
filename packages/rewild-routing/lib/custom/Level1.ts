import { ApplicationEventType, UIEventType, Listener, Event, degToRad } from "rewild-common";
...
export class Level1 extends Container implements Listener {
  ...
  init(): void {
    ...
    // GROUND PLANE
    this.groundBody = new Body(new BodyOptions().setMass(0).setShape(new Plane()).setMaterial(groundMaterial));
    this.groundBody!.quaternion.setFromAxisAngle(new Vec3Physics(1, 0, 0), -degToRad(90));
  }

  onEvent(event: Event): void {
    if (event.attachment instanceof ApplicationEvent) {
      const uiEvent = event.attachment as ApplicationEvent;
      if (uiEvent.eventType == ApplicationEventType.Quit) this.runtime!.sendSignal(this.getPortal("Exit")!, true);
    }
  }
