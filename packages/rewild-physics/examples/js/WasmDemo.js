// import * as CANNON from '../../build/index_bundle.js';
 ...
 */
class Demo extends EventDispatcher {
  sceneFolder;
  scenes = [];
  listeners = {};
  ...
  constructor(options) {
    ...
    // Init cannon.js
    this.world = new CANNON.ClientWorld();
  ...
  initGui = () => {
    ...
    solverFolder.add(this.settings, 'k', 10, 10000000).onChange((k) => {
      this.setGlobalSpookParams(
        this.settings.k,
        this.settings.d,
        1 / this.settings.stepFrequency
      );
    });
        ...
        switch (true) {
          ...
          case constraint instanceof CANNON.DistanceConstraint: {
            constraint.equations.forEach((equation) => {
              ...
              line.scale.set(
                vector.x - bi.position.x,
                vector.y - bi.position.y,
                vector.z - bi.position.z
              );
              makeSureNotZero(line.scale);
          ...
          case constraint instanceof CANNON.PointToPointConstraint: {
            constraint.equations.forEach((equation) => {
              ...
              if (equation.penetrationVec) {
                diffLine.scale.set(
                  -equation.penetrationVec.x,
                  -equation.penetrationVec.y,
                  -equation.penetrationVec.z
                );
              }
  ...
  initSmoothie = () => {
    ...
    this.world.addEventListener('postStep', this);
  ...
  addVisualsInstanced(bodies) {
    if (
      ...
      !bodies.every(
        (body) => body instanceof CANNON.Body && body.type === bodies[0].type
      )
    ) {
      throw new Error(
        'The argument passed to addVisualsInstanced() is not an array of bodies of the same type'
      );
    }
...
class GeometryCache {
  ...
  request = () => {
    const geometry =
      this.geometries.length > 0 ? this.geometries.pop() : this.createFunc();
