...
export class RaycastVehicle implements Listener {
   ...
   */
  addWheel(options: WheelInfoOptions = new WheelInfoOptions()): i32 {
  ...
  updateVehicle(timeStep: f32): void {
    ...
    for (let i: i32 = 0; i < numWheels; i++) {
      ...
      // Lock wheels
      if (Mathf.abs(wheel.brake) > Mathf.abs(wheel.engineForce)) {
  ...
  updateSuspension(deltaTime: f32): void {
    ...
    for (let w_it: i32 = 0; w_it < numWheels; w_it++) {
   ...
   */
  getWheelTransformWorld(wheelIndex: i32): Transform {
    return this.wheelInfos[wheelIndex].worldTransform;
  }

  updateFriction(timeStep: f32): void {
    ...
    for (let i: i32 = 0; i < numWheels; i++) {
      ...
      if (groundObject) {
        ...
        if (impulseSquared > maximpSquared) {
          ...
          const factor = maximp / Mathf.sqrt(impulseSquared);
...
function computeImpulseDenominator(body: Body, pos: Vec3, normal: Vec3): f32 {
