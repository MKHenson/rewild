...
CANNON.Demo = function (options) {
  ...
  var particleMaterial = (this.particleMaterial = new THREE.MeshLambertMaterial(
    { color: 0xff0000 }
  ));
  ...
  function updateVisuals() {
    ...
    if (settings.constraints) {
      ...
      for (var ci = 0; ci < world.constraints.length; ci++) {
        ...
        var bi = nc.bi,
          bj = nc.bj,
          line = distanceConstraintMeshCache.request();
        var i = bi.id,
          j = bj.id;
      ...
      for (var ci = 0; ci < world.constraints.length; ci++) {
        ...
        var bi = n.bi,
          bj = n.bj,
          relLine1 = p2pConstraintMeshCache.request(),
          relLine2 = p2pConstraintMeshCache.request(),
      ...
      for (var i = 0; i < bodies.length; i++) {
        ...
        if (b.computeAABB) {
          ...
          if (
            ...
            mesh.scale.set(
              b.aabb.lowerBound.x - b.aabb.upperBound.x,
              b.aabb.lowerBound.y - b.aabb.upperBound.y,
              b.aabb.lowerBound.z - b.aabb.upperBound.z
            );
            mesh.position.set(
              (b.aabb.lowerBound.x + b.aabb.upperBound.x) * 0.5,
  ...
  function init() {
    ...
    if (window.dat != undefined) {
      ...
      wf.add(settings, "gy", -maxg, maxg).onChange(function (gy) {
        if (!isNaN(gy)) world.gravity.set(settings.gx, gy, settings.gz);
      });
      wf.add(settings, "gz", -maxg, maxg).onChange(function (gz) {
        if (!isNaN(gz)) world.gravity.set(settings.gx, settings.gy, gz);
      });
      wf.add(settings, "quatNormalizeSkip", 0, 50)
        .step(1)
        .onChange(function (skip) {
          if (!isNaN(skip)) {
      ...
      sf.add(settings, "iterations", 1, 50)
        .step(1)
        .onChange(function (it) {
          world.solver.iterations = it;
        });
      sf.add(settings, "k", 10, 10000000).onChange(function (k) {
        that.setGlobalSpookParams(
          settings.k,
          settings.d,
          1 / settings.stepFrequency
        );
      });
      sf.add(settings, "d", 0, 20)
        .step(0.1)
        .onChange(function (d) {
          that.setGlobalSpookParams(
            settings.k,
      ...
      sf.add(settings, "tolerance", 0.0, 10.0)
        .step(0.01)
        .onChange(function (t) {
...
CANNON.Demo.prototype = new window.EventDispatcher();
...
CANNON.Demo.prototype.shape2mesh = function (body) {
  ...
  for (var l = 0; l < body.shapes.length; l++) {
    ...
    switch (shape.type) {
      case CANNON.Shape.SPHERE:
        var sphere_geometry = new THREE.SphereGeometry(shape.radius, 8, 8);
        mesh = new THREE.Mesh(sphere_geometry, this.currentMaterial);
        break;

      case CANNON.Shape.PARTICLE:
        mesh = new THREE.Mesh(this.particleGeo, this.particleMaterial);
        var s = this.settings;
        mesh.scale.set(s.particleSize, s.particleSize, s.particleSize);
        break;

      case CANNON.Shape.PLANE:
      ...
      case CANNON.Shape.CONVEXPOLYHEDRON:
